...
def get_news_recommendation(user_profile, vectorstore_category, number_of_articles):
    ...
    file_path = ""
    if (vectorstore_category == "denison_edu"):
        file_path = "news-denison-edu.json"

    if (vectorstore_category == "denisonian"):
        file_path = "news-the-denisonian.json"

    with open(f"./test-data/{file_path}", "r", encoding="utf8") as f:
        data = json.load(f)
        return get_openai_article_recommendation(data, user_profile, set_the_tone_complete, vectorstore_category)
